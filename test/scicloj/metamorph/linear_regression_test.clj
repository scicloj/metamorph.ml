...
(defn validate-model--linear-regression-mtcars-fm-ols [ds model]
  ...
  (let [glance (-> (ml/glance model) (ds/rows :as-map) first)
    ...
    (is (approx?   1126.0471874999998 (-> glance :totss)))
    (is (approx?   163.70981043447966 (-> glance :aic)))
    (is (approx?   181.29864126807638 (-> glance :bic)))
    (is (approx?   -69.85490521723983 (-> glance :log-lik)))
    (is (approx?    97.85527574833492 (:mse glance)))
    (is (approx?     3.7931521057466E-7 (:p.value glance)))
    (is (=    10 (:df glance)))
    (is (=    32 (:n glance)))
    ...
    (is (all-approx? [12.303374155996154
                      ...
                      -0.1994192548562663]
                     (-> tidy :estimate)))
    (is (all-approx? [18.71788442872143
                      1.0450233625100218
                      ...
                      0.8287524976817674]
                     (-> tidy :std.error)))
    (is (all-approx? [0.5181243968984746
                      0.9160873755159584
                      0.46348865035386666
                      0.33495531411697543
                      0.6352778979694993
                      ...
                      0.8121787129526958]
                     (-> tidy :p.value)))
    (is (all-approx? [0.657305808402011
                      -0.1066392215569186
                      0.7467584868409959
                      -0.986840654126232
                      0.48130361649375314
